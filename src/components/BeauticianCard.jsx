...
export default function BeauticianCard({ beautician, distance }) {
  // Helper function to get image URL (NEW)
  const getImageUrl = (imagePath) => {
    if (!imagePath) return '/default-beautician.jpg';
    if (imagePath.startsWith('http')) return imagePath;
    // For Cloudinary URLs or other external URLs
    return imagePath;
  };
  ...
  return (
    <div className="bg-white rounded-lg shadow-md overflow-hidden hover:shadow-lg transition">
      <div className="relative h-48">
        <img
          src={getImageUrl(beautician.portfolio?.[0] || beautician.profilePhoto)}
          alt={beautician.userId.name}
          className="w-full h-full object-cover"
          onError={(e) => {
            e.target.src = '/default-beautician.jpg';
          }}
        />
